    ...
    <div class="container mt-5">
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-primary text-white">
                <h2 class="m-0">Chi tiết đơn hàng #{{ $order->id }}</h2>
            </div>
            <div class="card-body">
                <p><strong>Người đặt:</strong> {{ $order->user->name_user ?? 'khách đặt online' }}</p>
                <p><strong>Tỉnh/Thành:</strong> {{ $order->province }}</p>
                <p><strong>Quận/Huyện:</strong> {{ $order->district }}</p>
                <p><strong>Xã/Phường:</strong> {{ $order->ward }}</p>
                <p><strong>Địa chỉ:</strong> {{ $order->detail_address }}</p>
                <p><strong>Số điện thoại:</strong> {{ $order->phone }}</p>
                <p><strong>Trạng thái:</strong>
                    <span class="badge badge-status" data-status="{{ $order->status }}">
            ...
            <ul class="list-group list-group-flush">
                @foreach ($order->orderItems as $item)
                    ...
                    <li class="list-group-item d-flex align-items-center">
                        <img src="{{ asset('storage/' . ($item->product->productImages->where('colour_id', $item->color_id)->first()->image_path ?? $item->product->productImages->first()->image_path ?? 'default-image.jpg')) }}"
                        class="product-image mr-3" />
                        <div class="flex-grow-1">
                            <strong class="large-text">{{ $item->product->name_sp }}</strong>

                            <div class="small text-muted">
                                <span class="large-text" style="color: #dc3545">Số lượng: {{ $item->quantity }}</span> |
                                <span class="large-text"
                                    style="color: green">Pin:{{ $item->product->battery->capacity }}</span> |

                                <span class="large-text" style="color: blue">Màu: {{ $item->color->name }}</span> |
                                <span class="large-text">Ram: {{ $item->variant->ram_smartphone ?? 0 }}</span>
                        ...
                        <span class="text-primary font-weight-bold large-text">
                            ...
                            @else
                                {{ number_format($order->total_price - $shipping_fee, 0, ',', '.') }} ₫
                            @endif
                        </span>
                ...
                @endforeach
                <li>
                    @if ($order->discount_code || $order->discount_percentage > 0 || $order->discount_amount > 0)
                <li class="list-group-item bg-light">
                    <div class="">
                        <strong class="text-muted">Mã giảm giá: </strong>
                        <span class="text-primary">
                            {{ $order->discount_code ?? 'Không có' }} |
                        </span>
                    </div>
                    <div class="">
                ...
                <li class="list-group-item d-flex justify-content-between">
                    ...
                    <span>@if ($order->payment_method == 'cash')
                        Tiền mặt
                    ...
                    <span class="text-danger font-weight-bold" style="font-size: 18px;">
                        @if ($order->user_id)
                            @if ($order->discount_amount == 0)
                                {{ number_format($order->total_after_discount + $order->shipping_fee, 0, ',', '.') }} ₫
                            @else
