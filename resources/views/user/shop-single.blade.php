                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-7">
                                <div class="product-details-des">
                                    ...
                                    <h3 class="product-name">{{ $product->name_sp }}</h3>
                                    {{-- Hiển thị cái thanh đánh giá --}}
                                    ...
                                    <p class="pro-desc">Dung lượng pin: {{ $product->battery->capacity }}</p>
                                            ...
                                            @foreach ($productVariants as $productVariant)
                                                <div class="me-2">
                                                    <button class="btn2 btn-outline-primary variant-button"
                                                        data-price="{{ $productVariant->variant->price }}"
                                                        data-id="{{ $productVariant->variant->id }}"
                                                        data-quantity="{{ $productVariant->quantity }}">
                                                        {{ $productVariant->variant->ram_smartphone }}
                                                    </button>
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="quantity-cart-box d-flex align-items-center">
                                        ...
                                        @if ($product->colours->count() > 0)
                                            <span id="stockQuantity"
                                                style="color: red">{{ $product->getTotalColourQuantityAttribute() }}</span>
                                        @else
                                    ...
                                    <div class="price-box">Giá:
                                        <span class="price-regular"
                                            id="totalPrice">{{ number_format($product->price, 0, ',', '.') }}
                                            ₫</span>
                                    </div>
                                    <div class="quantity-cart-box d-flex align-items-center">
                                        ...
                                        <div class="quantity">
                                            <div class="diamond-container">
                                                ...
                                                <input disabled type="text" id="quantity" value="1" min="1"
                                                    max="{{ $product->getTotalColourQuantityAttribute() }}"
                                    ...
                                    <div class="useful-links">
                                        <a href="javascript:void(0);" data-bs-toggle="tooltip" title="Wishlist"
                                            class="add-to-wishlist" data-product-id="{{ $product->id }}">
                                            <i class="pe-7s-like"></i>Yêu thích</a>
                                    </div>
                                    <div class="like-icon">
                                                        ...
                                                        <div class="review-header">
                                                            @if ($review->user->image)
                                                                <img src="{{ asset($review->user->image) }}"
                                                                    alt="{{ $review->user->name_user }}"
                                                                    class="review-avatar">
                                                            @else
                                                                <img src="{{ asset('asset-admin/img/profile.jpg') }}"
                                                                    alt="Avatar mặc định" class="review-avatar">
                                                            @endif

                                                            <strong>{{ $review->user->name_user }}</strong>
                                                            {{-- <span class="review-email">({{ $review->email }})</span> --}}
                                                            <div class="review-rating">
                                                                @for ($i = 1; $i <= 5; $i++)
                                                                    <i
                                                                        class="fa {{ $i <= $review->rating ? 'fa-star' : 'fa-star-o' }}"></i>
                                                                @endfor
                                                            </div>
                                                            ...
                                                            <span class="product-info">
                                                                {{-- Hàng sản phẩm: {{ $review->product->name }} |  --}}
                                                                Phân loại hàng:
                                                                {{ $review->product->category->name ?? 'Chưa phân loại' }}
                                                            </span>
                                                        </div>
                            ...
                            <div class="product-item">
                                <figure class="product-thumb">
                                    <a href="{{ route('product.show', $related->id) }}">
                                        <img class="pri-img" src="{{ asset('storage/' . $related->image) }}"
                                            alt="product">
                                        <img class="sec-img" src="{{ asset('storage/' . $related->image) }}"
                                            alt="product">
                                    </a>
                                ...
                                <div class="product-caption text-center">
                                    ...
                                    <h6 class="product-name">
                                        <a
                                            href="{{ route('product.show', $related->id) }}">{{ $related->name_sp }}</a>
                                    </h6>
                                    <div class="price-box">
                                        <span class="price-regular">{{ number_format($related->price, 0, ',', '.') }}
                                            ₫</span>
...
<script>
    ...
    $(document).ready(function() {
        ...
        let maxQuantity = {{ $product->getTotalColourQuantityAttribute() }};

        // Kiểm tra nếu sản phẩm là Điện thoại
        let isPhoneCategory = '{{ $product->category->name }}' === 'Điện thoại';
        ...
        function updatePrice() {
            let totalPrice = basePrice + selectedVariantPrice  + selectedColourPrice;
        ...
        if (isPhoneCategory && !selectedVariantId) {
        ...
        return;
            ...
            // Kiểm tra tồn kho
            if (maxQuantity === 0) {
                showToastr('error', 'Sản phẩm đã hết hàng!');
                return;
            }
