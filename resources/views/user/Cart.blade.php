                        ...
                        <div class="col-lg-12">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($cart->cartItems as $item)
                                            ...
                                            ?>
                                            <tr style="{{ $isOutOfStock ? 'opacity: 0.5;' : '' }}">
                                                <td>
                                                    <input type="checkbox" class="product-checkbox"
                                                        onclick="updateGrandTotal()" data-id="{{ $item->id }}"
                                                        {{ $isOutOfStock ? 'disabled' : '' }}>
                                                </td>
                                                <td class="pro-thumbnail">
                                                    <a href="#">
                                                        <img class="img-fluid"
                                                            src="{{ asset('storage/' . $item->product->productImages->where('colour_id', $item->color_id)->first()->image_path) }}"
                                                            alt="Product" />
                                                    </a>
                                                </td>
                                                <td class="pro-title">
                                                    <a href="#">{{ $item->product->name_sp }}</a>
                                                </td>
                                                <td class="pro-color">{{ $item->color->name ?? 'Không có màu' }}</td>
                                                <td>{{ $item->product->battery->capacity }}</td>

                                                <td class="pro-ram">{{ $item->variant->ram_smartphone ?? 'Không có' }}
                                                </td>
                                                <td class="pro-price" data-price="{{ $totalPricePerItem }}">
                                                    {{ number_format($totalPricePerItem, 0, ',', '.') }} ₫
                                                </td>
                                                <td id="cart-item-{{ $item->id }}">
                                                    @if (!$isOutOfStock)
                                                ...
                                                <td class="pro-remove">
                                                    <a href="#"
                                                        onclick="confirmRemoveFromCart({{ $item->id }})"><i
                            ...
                            <div class="cart-update-option d-block d-md-flex justify-content-between">
                                <div class="apply-coupon-wrapper">
                                    <form id="discount-form" action="{{ route('cart.applyDiscount') }}" method="POST"
                                ...
                                <div class="cart-page-total">
                                    <ul>
                                        ...
                                        <li id="discount-li"
                                            style="display: {{ session()->has('discount_code') && session('total_after_discount') > 0 ? 'list-item' : 'none' }}">
                                            Số tiền giảm giá
                                            <span
                                                id="discount-amount">{{ number_format(session('discount_amount', 0), 0, ',', '.') }}
                                                ₫</span>
                                        </li>
                                        ...
                                        <li id="total-after-discount-li"
                                            ...
                                            <span
                                                id="total-after-discount">{{ number_format(session('total_after_discount', 0), 0, ',', '.') }}
                                                ₫</span>
                                        </li>
                                    </ul>
                                </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-9">
                                    <a href="{{ route('shop') }}" class="btn btn-sqr">Thêm sản phẩm</a>
                                </div>
                                <div class="col-md-3">
                                    <div class="cart-update">
...
<script>
    document.querySelector('#checkout-button').addEventListener('click', function(e) {
        ...
        $.ajax({
            url: "{{ route('cart.checkStock') }}", // Tạo route kiểm tra số lượng
            type: 'POST',
            data: {
