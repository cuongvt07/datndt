                                ...
                                @if ($order->status == 'completed')
                                    @foreach ($order->orderItems as $item)
                                        <button class="btn btn-sqr mt-2" data-bs-toggle="modal"
                                            data-bs-target="#reviewModal" data-product-id="{{ $item->product->id }}">
                                            Đánh giá
                                        </button>
                                    @endforeach
                                @endif
                ...
                @foreach ($order->orderItems as $item)
                    <div class="sanpham">
                        ...
                        <div class="product-info">
                            ...
                            <span class="category-info">Phân loại hàng:
                                {{ $item->product->category->name ?? 'Chưa phân loại' }}</span>
                        </div>
                    </div>
                    <form action="{{ route('reviews.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" id="form-btn1"
                                data-bs-dismiss="modal">Hủy</button>
                            <button type="submit" class="btn btn-sqr" id="form-btn"
                                onclick="showSuccessMessage()">Gửi đánh giá</button>
                        </div>
                    </form>
                @endforeach
            </div>
