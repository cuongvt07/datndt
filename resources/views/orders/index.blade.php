...
<div class="container mt-5">
    ...
    <a href="{{ route('orders.completed-canceled') }}" class="btn btn-primary btn-lg px-4 py-2 rounded-pill shadow-sm">
        Đơn Hàng Đã Hủy
    </a>
            ...
            <tbody>
                @foreach ($orders as $order)
                    <tr>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="cancelModalLabel-{{ $order->id }}">Hủy đơn hàng
                                            #{{ $order->id }}</h5>
                                        ...
                                        <form action="{{ route('orders.cancel', ['order' => $order->id]) }}"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="checkbox" name="reasons[]" value="Khác"
                                                    id="reason-other-{{ $order->id }}"> Khác<br>
                                            </div>

                                            <!-- Ô nhập lý do khác -->
                                            <div class="form-group" id="other-reason-input-{{ $order->id }}"
                                                style="display: none;">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="editAddressModal-{{ $order->id }}" tabindex="-1"
                            aria-labelledby="editAddressModalLabel-{{ $order->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editAddressModalLabel-{{ $order->id }}">Cập nhật
                                            địa chỉ</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="{{ route('orders.updateAddress', ['order' => $order->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-md-12">
                                                    ...
                                                    <div id="edit-address-error-{{ $order->id }}"
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-md-4">
                                                    <select id="tinh-{{ $order->id }}" name="province"
                                                ...
                                                <div class="col-md-4">
                                                    <select id="quan-{{ $order->id }}" name="district"
                                                ...
                                                <div class="col-md-4">
                                                    <select id="phuong-{{ $order->id }}" name="ward"
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-md-12">
                                                    ...
                                                    <input type="text" class="form-control"
                                                        id="phone-{{ $order->id }}" name="phone"
                                                        value="{{ $order->phone }}" required>
                                                    <div id="edit-phone-error-{{ $order->id }}"
                        ...
                        <td>
                            @if ($order->orderItems->count() > 0)
                                <div class="align-items-center mb-2">
                                    ...
                                    <img src="{{ asset('storage/' .$order->orderItems->first()->product->productImages->where('colour_id', $order->orderItems->first()->color_id)->first()->image_path ??'default-image.jpg') }}"
                                        alt="Product Image"
                                        style="max-width: 100px; max-height: 100px; object-fit: cover;"
                                        loading="lazy">

                                    <div>
                                        <strong>{{ $order->orderItems->first()->product->name_sp }}</strong> <br>
                                        <!-- Display the total quantity of the first product -->
                                        <small>Số sản phẩm: {{ $order->orderItems->sum('quantity') }}</small>
                                    </div>

                                    <!-- Modal button to view all products in the order -->
                                    <button class="btn btn-info btn-sm mt-2" data-bs-toggle="modal"
                                        data-bs-target="#productDetailModal-{{ $order->id }}">Xem chi tiết</button>
                                </div>
                            @endif

                            <!-- Modal for showing detailed information of all products -->
                            <div class="modal fade" id="productDetailModal-{{ $order->id }}" tabindex="-1"
                                aria-labelledby="productDetailModalLabel-{{ $order->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="productDetailModalLabel-{{ $order->id }}">
                                                ...
                                                @foreach ($order->orderItems as $item)
                                                    <div class="product-details card shadow-sm mb-4"
                                                        ...
                                                        <div class="card-img-top text-center p-3">
                                                            <img src="{{ asset('storage/' . $item->product->productImages->where('colour_id', $item->color_id)->first()->image_path ?? 'default-image.jpg') }}"
                                                                alt="Product Image for {{ $item->product->name_sp }}"
                                                                class="product-image"
                                                        ...
                                                        <!-- Product Details -->
                                                        <div class="card-body">
                                                            <h6 class="card-title">{{ $item->product->name_sp }}</h6>
                                                            ...
                                                            <div class="product-info-collapse">
                                                                <div class="product-info">
                                                                    <strong>Pin:</strong>
                                                                    <span>{{ $item->product->battery->capacity }}</span>
                                                                </div>
                                                                <div class="product-info">
                                                                    <strong>RAM:</strong>
                                                                    <span>{{ $item->variant->ram_smartphone ?? 'Không có' }}</span>
                                                                </div>
                                                                <div class="product-info">
                                                                ...
                                                                <div class="product-info">
                                                                    <strong>Mô tả:</strong>
                                                                    <p>{{ \Illuminate\Support\Str::limit($item->product->description, 80, '...') }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
                        ...
                        <td>
                            @if ($order->discount_amount == 0)
                                <strong
                                    style="font-size: 16px">{{ number_format($order->total_after_discount + $order->shipping_fee, 0, ',', '.') }}
                                    ₫</strong>
                            @else
                                <strong
                                    style="font-size: 16px; color: red;">{{ number_format($order->total_after_discount + $order->shipping_fee, 0, ',', '.') }}
                                    ₫</strong>
                                <br>
                                <span
...
<!-- Bootstrap JS -->
<script src="{{ asset('asset-user/js/vendor/bootstrap.bundle.min.js') }}"></script>
<!-- Countdown JS -->
<script src="{{ asset('asset-user/js/plugins/countdown.min.js') }}"></script>
<!-- Nice Select JS -->
<script src="{{ asset('asset-user/js/plugins/nice-select.min.js') }}"></script>
<!-- jquery UI JS -->
<script src="{{ asset('asset-user/js/plugins/jqueryui.min.js') }}"></script>
<!-- Image zoom JS -->
<script src="{{ asset('asset-user/js/plugins/image-zoom.min.js') }}"></script>
<!-- Images loaded JS -->
<script src="{{ asset('asset-user/js/plugins/imagesloaded.pkgd.min.js') }}"></script>
<!-- mail-chimp active js -->
<script src="{{ asset('asset-user/js/plugins/ajaxchimp.js') }}"></script>
<!-- contact form dynamic js -->
<script src="{{ asset('asset-user/js/plugins/ajax-mail.js') }}"></script>
<script src="{{ asset('asset-user/js/plugins/google-map.js') }}"></script>
<!-- Main JS -->
...
<script>
            ...
            if (data_tinh.error == 0) {
                ...
                @foreach ($orders as $order)
                    $.each(data_tinh.data, function(key_tinh, val_tinh) {
                        ...
                        $("#tinh-{{ $order->id }}").append('<option value="' + val_tinh.id +
                            '">' + val_tinh.full_name + '</option>');
                    });

                    // Xử lý sự kiện khi thay đổi tỉnh cho từng đơn hàng
                    $("#tinh-{{ $order->id }}").change(function(e) {
                        var idtinh = $(this).val();
                        ...
                        $.getJSON('https://esgoo.net/api-tinhthanh/2/' + idtinh + '.htm',
                            function(data_quan) {
                                if (data_quan.error == 0) {
                                    ...
                                    $("#quan-{{ $order->id }}").change(function(e) {
                                        var idquan = $(this).val();
    ...
    $(document).ready(function() {
        @foreach ($orders as $order)
            $('#reason-other-{{ $order->id }}').change(function() {
                if ($(this).is(':checked')) {
                    $('#other-reason-input-{{ $order->id }}').show();
                } else {
                    $('#other-reason-input-{{ $order->id }}').hide();
                }
            });
        @endforeach
    });
                        ...
                        <div class="widget-title">
                            <div class="logo">
                                <a href="" style="text-decoration: none;">
                                    <h1 style="font-size: 24px; font-weight: bold; color: #ff5722;">StorePhone</h1>
                                </a>
                            </div>
                        </div>
            ...
            <div class="row align-items-center mt-20">
                ...
                <div class="col-md-6">
                    <div class="footer-payment">
                        <img src="{{ asset('asset-user/img/payment.png') }}" alt="Phương thức thanh toán">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="copyright-text text-center">
                        <p>&copy; 2024 <b>StorePhone</b> Thiết kế với <i class="fa fa-heart text-danger"></i> bởi
                            NhomWD01.</p>
                    </div>
                </div>
            </div>
