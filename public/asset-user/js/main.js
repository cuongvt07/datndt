    ...
    $(document).ready(function () {
        ...
        function formatCurrencyVND(amount) {
            return new Intl.NumberFormat("vi-VN", {
                style: "currency",
                currency: "VND",
                minimumFractionDigits: 0, // Số thập phân tối thiểu
                maximumFractionDigits: 0, // Số thập phân tối đa
            }).format(amount);
        }

        var rangeSlider = $(".price-range"),
            amount = $("#amount"),
            minPrice = rangeSlider.data("min") * 1000000,
            maxPrice = rangeSlider.data("max") * 1000000,
            minPriceEl = $("#minPrice"),
            maxPriceEl = $("#maxPrice");
        if (minPricePHP && maxPricePHP) {
            console.log("123");
            rangeSlider.slider({
                range: true,
                min: minPrice,
                ...
                slide: function (event, ui) {
                    amount.val(
                        formatCurrencyVND(ui.values[0]) +
                            " - " +
                            formatCurrencyVND(ui.values[1])
                    );
                    console.log(minPriceEl);
                    minPriceEl.val(ui.values[0]);
                    maxPriceEl.val(ui.values[1]);
                },
            });

            amount.val(
                formatCurrencyVND(rangeSlider.slider("values", 0)) +
                    " - " +
                    formatCurrencyVND(rangeSlider.slider("values", 1))
            );
        } else {
            rangeSlider.slider({
                range: true,
